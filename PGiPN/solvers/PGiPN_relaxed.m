...
%% by using PGiPN with relax switch condition, 
%% a hybrid of Proximal Gradient and inexact Project regularized Newton method with relax switch condition.
%% ***************************************************************************

function [out] = PGiPN_relaxed(prob,options)
...
for iter = 1:maxiter
    ...
    if search_num1 > 2 || old_search1 > 2
        tau = 10;
    else
        tau = 2;
    end
    old_search1 = search_num1;
    
    [T_new, Tsignabs] = reg.Bxsupp(xnew); T_nnz = sum(abs(T_new)>0); 
    [S_new, Ssignabs] = reg.xsupp(xnew); S_nnz = sum(abs(S_new)>0); 
    
    if iter > 2 && sum(abs(Tsignabs - Tsignabso)) < (0.01*n)/iter && sum(abs(Ssignabs - Ssignabso))<(0.01*n)/iter
    ...
    if cond
        ...
        model.sense = '=';
        params.outputflag = 0;
        params.Cutoff = -0.5*u'*G*u + rgradf'*u;
